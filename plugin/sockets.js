(function (Sockets) {
    ...
    Sockets.createAward = function (socket, payload, callback) {
        async.waterfall([
            ...
            function (file, next) {
                    database.createAward(payload.name, payload.desc, uploads.getPublicImagePath(file), payload.type, payload.cond, payload.condval, payload.reason, payload.limit, payload.userLimit, next);
            }
        ], callback);
    };
    ...
    Sockets.editAward = function (socket, payload, callback) {
        controller.editAward(payload.id, payload.name, payload.desc, payload.image, payload.type, payload.cond, payload.condval, payload.reason, payload.limit, payload.userLimit, callback);
    };
